...
$students = [];
$result = $conn->query("SELECT id, first_name, last_name, class FROM students ORDER BY class, last_name");
...
// Fetch all fee types
$fee_types = [];
$result = $conn->query("SELECT DISTINCT fee_type FROM fee_structure");
while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO student_fees (student_id, fee_type, amount, payment_date, payment_method, receipt_number, notes, recorded_by) 
                           VALUES (:student_id, :fee_type, :amount, :payment_date, :payment_method, :receipt_number, :notes, :recorded_by)");
    
    $stmt->bindValue(':student_id', $student_id, SQLITE3_INTEGER);
    $stmt->bindValue(':fee_type', $fee_type, SQLITE3_TEXT);
    $stmt->bindValue(':amount', $amount, SQLITE3_FLOAT);
    $stmt->bindValue(':payment_date', $payment_date, SQLITE3_TEXT);
    $stmt->bindValue(':payment_method', $payment_method, SQLITE3_TEXT);
    $stmt->bindValue(':receipt_number', $receipt_number, SQLITE3_TEXT);
    $stmt->bindValue(':notes', $notes, SQLITE3_TEXT);
    $stmt->bindValue(':recorded_by', $_SESSION['id'], SQLITE3_INTEGER);
